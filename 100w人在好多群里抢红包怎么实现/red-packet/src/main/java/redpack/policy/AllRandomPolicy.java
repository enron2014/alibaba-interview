...
@Component("allRandomPolicy")
public class AllRandomPolicy implements RedPacketPolicy {

	private static final String KEY = "AR_RED_PACK";
	...
	public AllRandomPolicy(LuaExecutor luaExecutor) {
	...
	private DefaultRedisScript<Integer> getInitScript() {
		...
		script.setLocation(new ClassPathResource("lua/allRandom/initial.lua"));
		return script;
	}

	private DefaultRedisScript<Integer> getGrabScript() {
		DefaultRedisScript<Integer> script = new DefaultRedisScript<>();
		script.setLocation(new ClassPathResource("lua/allRandom/grab.lua"));
		return script;
	}
